...
class Recetario{
    ...
    object Singleton {
        var recetario: MutableList<Receta> = mutableListOf(
            Receta(id = 1,
        ...
        var listacompra: MutableList<Receta> = mutableListOf()
    }
    ...
    fun getListaCompra(): MutableList<Receta>{
        return Singleton.listacompra
    }

    fun anadirListaCompra(r: Receta): Boolean{
        val i = Singleton.listacompra.indexOf(r)

        if (i == -1){
            Singleton.listacompra.add(r)
            return true
        }
        else {
            return false
    ...
    fun quitarListaCompra(r: Receta){
        Singleton.listacompra.remove(r)
    }

    @SuppressLint("DefaultLocale")
    fun search(query: String): MutableList<Receta>{
        val search_list: MutableList<Receta> = mutableListOf()

        val size = Singleton.recetario.size - 1

        for (i: Int in (0..size)){
            Log.i("SEARCH", query + " " + Singleton.recetario[i].getNombre())
            if (Singleton.recetario[i].getNombre().toLowerCase().contains(query.toLowerCase())) {
                Log.i("SEARCH", Singleton.recetario[i].getNombre())
                search_list.add(Singleton.recetario[i])
            }
        }
    ...
    fun getReceta(id: Int): Receta{
        ...
        for (i: Int in (0..size)){
            if (Singleton.recetario[i].getId() == id) return Singleton.recetario[i]
        }

        return Singleton.recetario[0]
    }
    ...
    fun setDone(id: Int){
        ...
        for (i: Int in (0..size)){
            if (Singleton.recetario[i].getId() == id) Singleton.recetario[i].setDone(true)
        }
    }
}